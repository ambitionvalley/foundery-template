...
const docRows: DocRow[] = [
  {
    ...
    body: (
      <p>
        Search Popup is a command-palette-style overlay. Use it as a global
        keyboard-driven entry point for search, navigation, and contact
        look-ups. The popover is 600px wide and groups results under labelled
        sections (Recent search, Recently visited, Contacts by default).
      </p>
    ),
  },
  {
    ...
    body: (
      <p>
        Surface is 90% white with a 40px backdrop blur and a 28px drop shadow,
        so it floats cleanly over any background. Highlight one item per
        section to signal keyboard focus, and render the return-arrow hint on
        that row to suggest the Enter key.
      </p>
    ),
    divider: "emphasis",
  },
...
/** 1:1 reproduction of the Figma symbol (600×768). */
    ...
    <section className="flex w-full flex-wrap items-start gap-x-1 gap-y-4 border-b border-b-black/10 py-[28px]">
      ...
      <div className="flex min-w-[358px] flex-1 flex-col items-start overflow-x-auto">
        <div
          ...
          style={{ width: 660, height: 820 }}
        >
          <div className="absolute" style={{ left: 30, top: 26 }}>
            <SearchPopup />
          </div>
        </div>
      </div>
...
function PropsTableRow() {
  const props = [
    { name: "value", type: "ReactNode", default: "—", description: "Typed value; empty shows the placeholder." },
    { name: "placeholder", type: "string", default: '"Search"', description: "Placeholder when empty." },
    { name: "sections", type: "SearchPopupSection[]", default: "Figma sample", description: "Groups of results. Default renders the Figma sample." },
...
export default function SearchPopupShowcasePage() {
  return (
          ...
          <div className="flex min-w-0 flex-1 flex-col gap-4">
            ...
            <p className="text-[14px] leading-[20px] text-black/40">
              <code className="rounded-[4px] bg-black/[0.04] px-[6px] py-[2px] font-mono text-[13px] text-black/70">
                @/components/common/search-popup
              </code>
            </p>
          </div>
          <BrandLogo priority />
