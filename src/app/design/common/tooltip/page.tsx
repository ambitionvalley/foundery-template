...
const docRows: DocRow[] = [
  {
    ...
    body: (
      <p>
        Tooltip is a compact label surfaced on hover to name an unlabeled
        control or disclose a keyboard shortcut. Two variants: a translucent
        dark gradient for image / dark surfaces, and a subtle 4% black pill
        for light surfaces.
      </p>
    ),
  },
  {
    ...
    body: (
      <p>
        Keep labels short — one or two words. Pair with a shortcut when the
        control has a keyboard binding. Tooltips are not a substitute for
        accessible labels; make sure the triggering control still has one.
      </p>
    ),
    divider: "emphasis",
  },
...
const VARIANTS: { y: number; variant: "dark" | "light" }[] = [
  { y: 20, variant: "dark" },
  { y: 54, variant: "light" },
];

/** 1:1 reproduction of the Figma canvas (112×98). */
      ...
      <div className="flex min-w-[358px] flex-1 flex-col items-start">
        ...
        >
          {VARIANTS.map((v) => (
            ...
            >
              <Tooltip variant={v.variant} shortcut="⌘" />
            </div>
          ))}
        </div>
      </div>
...
function PropsTableRow() {
  const props = [
    { name: "variant", type: '"dark" | "light"', default: '"dark"', description: "Dark gradient or light 4% pill." },
    { name: "children", type: "ReactNode", default: '"Tooltip"', description: "Primary label content." },
    { name: "shortcut", type: "ReactNode", default: "—", description: "Optional secondary text (keyboard shortcut)." },
