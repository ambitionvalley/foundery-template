...
import { QuickAction, type QuickActionProps } from "@/components/common/quick-action";
...
const docRows: DocRow[] = [
  {
    ...
    body: (
      <p>
        Quick Action is a floating menu card that presents a small column of
        equal-weight options. It is used next to a trigger — overflow menus,
        context menus, inline action pickers — and tops out at eight rows so
  ...
  },
  {
    ...
    body: (
      <p>
        ...
        it composes over any background. Each row has its own 8px padding and
        12px hover surface — rows share no border; visual grouping comes from
        the card alone.
      </p>
    ),
    divider: "emphasis",
...
// 1:1 reproduction of the Figma frame (1140×352): 8 variants, 120px wide,
// at x = 20, 160, 300, 440, 580, 720, 860, 1000 (140px pitch), y = 20.
const VARIANTS: { count: QuickActionProps["count"]; x: number }[] = [
  { count: 1, x: 20 },
  { count: 2, x: 160 },
  { count: 3, x: 300 },
  { count: 4, x: 440 },
    ...
    <section className="flex w-full flex-wrap items-start gap-x-1 gap-y-4 border-b border-b-black/10 py-[28px]">
      ...
      <div className="flex min-w-[358px] flex-1 flex-col items-start overflow-x-auto">
        <div
          ...
          style={{ width: 1140, height: 352 }}
        >
          {VARIANTS.map((v) => (
            <div key={v.count} className="absolute" style={{ left: v.x, top: 20 }}>
              <QuickAction count={v.count} />
            </div>
          ))}
        </div>
      </div>
...
function PropsTableRow() {
  const props = [
    { name: "options", type: "ReactNode[]", default: "—", description: "Option rows; overrides the default \"Option N\" fallback." },
    { name: "count", type: "1 | 2 | … | 8", default: "3", description: "Fallback row count when options is omitted." },
    { name: "onSelect", type: "(i: number) => void", default: "—", description: "When provided, each row becomes a button calling onSelect(index)." },
...
export default function QuickActionShowcasePage() {
  return (
          ...
          <div className="flex min-w-0 flex-1 flex-col gap-4">
            ...
            <p className="text-[14px] leading-[20px] text-black/40">
              <code className="rounded-[4px] bg-black/[0.04] px-[6px] py-[2px] font-mono text-[13px] text-black/70">
                @/components/common/quick-action
              </code>
            </p>
          </div>
          <BrandLogo priority />
