...
const docRows: DocRow[] = [
  {
    ...
    body: (
      <p>
        Tap is a tab-style label used to switch between peer views. Three
        states: <code>active</code> (underline + full-contrast label),
        <code>inactive</code> (40% black label), and <code>hover</code>
        (full-contrast label, no underline).
      </p>
    ),
  },
  {
    ...
    body: (
      <p>
        Group Taps in a horizontal row with consistent spacing. Only one Tap
        in a group should be <code>active</code> at a time. Combine with
        routing or view-state to make the selection meaningful.
      </p>
    ),
    divider: "emphasis",
  },
...
const VARIANTS: { x: number; state: "active" | "inactive" | "hover" }[] = [
  { x: 20, state: "active" },
  { x: 69, state: "inactive" },
  { x: 118, state: "hover" },
];

/** 1:1 reproduction of the Figma canvas (167×64). */
    ...
    <section className="flex w-full flex-wrap items-start gap-x-1 gap-y-4 border-b border-b-black/10 py-[28px]">
      ...
      <div className="flex min-w-[358px] flex-1 flex-col items-start">
        <div
          ...
          style={{ width: 167, height: 64 }}
        >
          {VARIANTS.map((v) => (
            <div
              key={v.state}
              className="absolute"
              style={{ left: v.x, top: 20 }}
            >
              <Tap state={v.state} />
            </div>
          ))}
...
function PropsTableRow() {
  const props = [
    { name: "state", type: '"active" | "inactive" | "hover"', default: '"active"', description: "Visual state." },
...
export default function TapShowcasePage() {
  return (
