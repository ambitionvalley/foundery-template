...
const docRows: DocRow[] = [
  {
    ...
    body: (
      <p>
        Notification is a toast-style banner for short transient feedback —
        confirmations, warnings, and error acknowledgements. Two semantic
        states (successful / failure) and two densities (big / small).
      </p>
    ),
  },
  {
    ...
    body: (
      <p>
        Surface is a translucent dark gradient (10% white over 80% black)
        with a 20px backdrop blur, so the banner sits cleanly over any
        background. Override the default text by passing children.
      </p>
    ),
    divider: "emphasis",
  },
...
const VARIANTS: { y: number; state: "successful" | "failure"; big: boolean }[] = [
  { y: 20, state: "successful", big: true },
  { y: 70, state: "failure", big: true },
  { y: 120, state: "successful", big: false },
  { y: 154, state: "failure", big: false },
];

/** 1:1 reproduction of the Figma canvas (214×198). */
      ...
      <div className="flex min-w-[358px] flex-1 flex-col items-start">
        ...
        >
          {VARIANTS.map((v) => (
            ...
            >
              <Notification state={v.state} big={v.big} />
            </div>
          ))}
        </div>
      </div>
...
function PropsTableRow() {
  const props = [
    { name: "state", type: '"successful" | "failure"', default: '"successful"', description: "Drives the icon and default message." },
    { name: "big", type: "boolean", default: "true", description: "Larger variant: 24px icon, 14/20 text." },
    { name: "children", type: "ReactNode", default: "—", description: 'Overrides the default "Done" / "Something Wrong" message.' },
