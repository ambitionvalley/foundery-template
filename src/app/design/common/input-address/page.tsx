...
import { InputAddress } from "@/components/common/input-address";
...
const docRows: DocRow[] = [
  {
    ...
    body: (
      <p>
        Address Input is a four-row stacked form that captures a postal
        address: country, street, state, and a split city / ZIP row. All
        edges coalesce into single 0.5px hairlines so the stack reads as
        one control.
      </p>
    ),
  },
  {
    ...
    body: (
      <p>
        The country and state rows expose a chevron-up-down affordance —
        wire them to a select / autocomplete popover. Leave the other
        values unset to show the 20%-black placeholder. Use in checkout
        flows, onboarding, and address-book rows.
      </p>
    ),
    divider: "emphasis",
  },
...
/** 1:1 reproduction of the Figma symbol (360×176). */
...
function PropsTableRow() {
  const props = [
    { name: "country", type: "ReactNode", default: '"United States"', description: "Country row value." },
    { name: "address", type: "ReactNode", default: "—", description: "Street address. Unset shows the placeholder." },
    { name: "state", type: "ReactNode", default: '"State"', description: "State / region value." },
    { name: "city", type: "ReactNode", default: "—", description: "City value. Unset shows the placeholder." },
    { name: "zip", type: "ReactNode", default: "—", description: "ZIP / postal code. Unset shows the placeholder." },
...
export default function InputAddressShowcasePage() {
  return (
          ...
          <div className="flex min-w-0 flex-1 flex-col gap-4">
            ...
            <p className="text-[14px] leading-[20px] text-black/40">
              <code className="rounded-[4px] bg-black/[0.04] px-[6px] py-[2px] font-mono text-[13px] text-black/70">
                @/components/common/input-address
              </code>
            </p>
          </div>
          <BrandLogo priority />
