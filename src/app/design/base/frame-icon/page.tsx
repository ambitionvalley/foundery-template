...
import { FrameIcon } from "@/components/base/frame-icon";
...
const docRows: DocRow[] = [
  {
    ...
    body: (
      <p>
        Single-icon slot — the simplest member of the Frame family. A 16×16
        icon wrapped in the same 12px-rounded hoverable surface used by the
        other Frame compositions.
      </p>
    ),
  },
  {
    ...
    body: (
      <p>
        Pass the icon via <code>icon</code>. Falls back to a dashed placeholder
        so layouts remain clear while icons are being authored.
      </p>
    ),
    divider: "emphasis",
  },
...
const SLOTS: { x: number; hover: boolean }[] = [
  { x: 20, hover: false }, // Default
  { x: 46, hover: true },  // Hover
  { x: 72, hover: false }, // Static
];

/** 1:1 reproduction of the Figma canvas (108×56). */
        ...
        >
          {SLOTS.map((slot) => (
            <div
              key={slot.x}
              className="absolute"
              style={{ left: slot.x, top: 20 }}
            >
              <FrameIcon hover={slot.hover} />
            </div>
          ))}
        </div>
      </div>
...
function PropsTableRow() {
  const props = [
    ...
    { name: "icon", type: "ReactNode", default: "—", description: "Icon content; falls back to a 16×16 dashed placeholder." },
...
export default function FrameIconShowcasePage() {
  return (
