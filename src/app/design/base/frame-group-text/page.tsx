...
import { FrameGroupText } from "@/components/base/frame-group-text";
...
const docRows: DocRow[] = [
  {
    ...
    body: (
      <p>
        Button group + label composition — a row where a cluster of icon
        buttons (leading or trailing) is paired with a descriptive label.
        Typical uses: toolbar rows, table headers with inline actions.
      </p>
    ),
  },
  {
    ...
    body: (
      <p>
        Pass the button cluster via <code>group</code> (any ReactNode — typically
        the <code>Group</code> primitive wrapping icon buttons). Use
        <code>flip</code> to send the cluster to the trailing edge, and
        <code>vertical</code> to stack.
      </p>
    ),
    divider: "emphasis",
  },
...
/** 1:1 reproduction of the Figma canvas (255×248). */
        ...
        >
          {H_ROWS.map((row) =>
            H_COLS.map((col) => (
              ...
              >
                <FrameGroupText flip={row.flip} hover={col.hover} />
              </div>
            )),
          )}
          {V_ROWS.map((row) =>
            V_COLS.map((col) => (
              <div
                ...
                style={{ left: col.x, top: row.y }}
              >
                <FrameGroupText vertical flip={row.flip} hover={col.hover} />
              </div>
            )),
          )}
        </div>
...
function PropsTableRow() {
  const props = [
    { name: "vertical", type: "boolean", default: "false", description: "Stack the button group and the label." },
    { name: "flip", type: "boolean", default: "false", description: "Group trails instead of leads." },
    { name: "hover", type: "boolean", default: "false", description: "Force the hover surface (showcase only)." },
    { name: "group", type: "ReactNode", default: "—", description: "Button cluster; falls back to a single dashed-icon borderless button." },
...
export default function FrameGroupTextShowcasePage() {
  return (
          ...
          <div className="flex min-w-0 flex-1 flex-col gap-4">
            ...
            <p className="text-[14px] leading-[20px] text-black/40">
              <code className="rounded-[4px] bg-black/[0.04] px-[6px] py-[2px] font-mono text-[13px] text-black/70">
                @/components/base/frame-group-text
              </code>
            </p>
          </div>
          <BrandLogo priority />
