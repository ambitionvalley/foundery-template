...
import { Tag, type TagVariant } from "@/components/base/tag";
...
const docRows: DocRow[] = [
  {
    ...
    body: (
      <p>
        Tag is a small inline pill label. Use it for chips, filters, metadata,
        and ribbon-style step indicators. Three variants: a rounded pill and
        two arrow-tipped ribbons.
      </p>
    ),
  },
  {
    ...
    body: (
      <p>
        Surface is <code>4%</code> black at rest, <code>10%</code> on hover.
        Pair <code>leadingDot</code> with a clear status label, and
        <code>trailingClose</code> with an <code>onClose</code> handler when
        the tag should be dismissible.
      </p>
    ),
    divider: "emphasis",
  },
...
const ROWS: RowConfig[] = [
  { variant: "default", leadingDot: true, trailingClose: true },
  { variant: "default", leadingDot: true },
  { variant: "default", trailingClose: true },
  { variant: "default" },
  { variant: "left-arrow" },
  { variant: "right-arrow" },
];

const ROW_Y = [20, 62, 104, 146, 188, 230];
const COLS: { x: number; hover: boolean }[] = [
  { x: 20, hover: false },
  { x: 102, hover: true },
  { x: 184, hover: false },
];

/** 1:1 reproduction of the Figma canvas (266×272). */
    ...
    <section className="flex w-full flex-wrap items-start gap-x-1 gap-y-4 border-b border-b-black/10 py-[28px]">
      <div className="flex min-h-[40px] min-w-[200px] max-w-[320px] flex-1 flex-col justify-start pt-2">
        ...
        <p className="mt-1 text-[12px] leading-[16px] text-black/40" style={{ fontFeatureSettings: "'ss01' 1, 'cv01' 1" }}>
          6 configurations · 3 states (default · hover · static)
        </p>
      </div>
          ...
          {ROWS.map((rowDef, rowIdx) =>
            COLS.map((col, colIdx) => (
              <div
                key={`${rowIdx}-${colIdx}`}
                className="absolute"
                style={{ left: col.x, top: ROW_Y[rowIdx] }}
              >
                <Tag
                  variant={rowDef.variant}
                  leadingDot={rowDef.leadingDot}
                  trailingClose={rowDef.trailingClose}
                  hover={col.hover}
                />
              </div>
            )),
...
function PropsTableRow() {
  const props = [
    { name: "variant", type: '"default" | "left-arrow" | "right-arrow"', default: '"default"', description: "Pill shape; arrow variants add a triangular tip." },
    { name: "leadingDot", type: "boolean", default: "false", description: "Round dot on the left (default variant only)." },
    { name: "trailingClose", type: "boolean", default: "false", description: "Close × on the right (default variant only)." },
    { name: "onClose", type: "(e: MouseEvent) => void", default: "—", description: "Makes the trailing × a button." },
