...
import { FrameIconTextIcon } from "@/components/base/frame-icon-text-icon";
...
const docRows: DocRow[] = [
  {
    ...
    body: (
      <p>
        Icon + label + icon composition — a three-slot row used for menu
        items with trailing chevrons, nav rows with badges, and list rows
        with actions.
      </p>
    ),
  },
  {
    ...
    body: (
      <p>
        Pass the label&apos;s icon via <code>icon</code> and the standalone
        icon via <code>endIcon</code>. Use <code>flip</code> to put the
        standalone icon on the leading side, and <code>vertical</code> to
...
// Horizontal variants live at x=20 / 107 / 194 (Default / Hover / Static).
// Vertical variants live at x=20 / 83 / 146 (Default / Hover / Static).
        ...
        >
          {H_ROWS.map((row) =>
            H_COLS.map((col) => (
              ...
              >
                <FrameIconTextIcon flip={row.flip} hover={col.hover} />
              </div>
            )),
          )}
          {V_ROWS.map((row) =>
            V_COLS.map((col) => (
              <div
                ...
                style={{ left: col.x, top: row.y }}
              >
                <FrameIconTextIcon vertical flip={row.flip} hover={col.hover} />
              </div>
            )),
          )}
        </div>
...
function PropsTableRow() {
  const props = [
    { name: "vertical", type: "boolean", default: "false", description: "Stack the icon+label group and the standalone icon." },
    ...
    { name: "icon", type: "ReactNode", default: "—", description: "Icon accompanying the label." },
...
export default function FrameIconTextIconShowcasePage() {
  return (
          ...
          <div className="flex min-w-0 flex-1 flex-col gap-4">
            ...
            <p className="text-[14px] leading-[20px] text-black/40">
              <code className="rounded-[4px] bg-black/[0.04] px-[6px] py-[2px] font-mono text-[13px] text-black/70">
                @/components/base/frame-icon-text-icon
              </code>
            </p>
          </div>
          <BrandLogo priority />
