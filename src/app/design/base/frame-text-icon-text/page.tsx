...
import { FrameTextIconText } from "@/components/base/frame-text-icon-text";
...
const docRows: DocRow[] = [
  {
    ...
    body: (
      <p>
        Label + icon + text composition — a three-slot row where the primary
        label leads its paired action icon, and a secondary standalone text
  ...
  },
  {
    ...
    body: (
      <p>
        ...
        standalone text, and <code>vertical</code> to stack the two groups.
      </p>
    ),
    divider: "emphasis",
  },
        ...
        >
          {H_ROWS.map((row) =>
            H_COLS.map((col) => (
              ...
              >
                <FrameTextIconText flip={row.flip} hover={col.hover} />
              </div>
            )),
          )}
          {V_ROWS.map((row) =>
            V_COLS.map((col) => (
              <div
                ...
                style={{ left: col.x, top: row.y }}
              >
                <FrameTextIconText vertical flip={row.flip} hover={col.hover} />
              </div>
            )),
          )}
        </div>
...
function PropsTableRow() {
  const props = [
    { name: "vertical", type: "boolean", default: "false", description: "Stack the label+icon pair and the standalone text." },
    ...
    { name: "icon", type: "ReactNode", default: "—", description: "Icon that follows the primary label." },
...
export default function FrameTextIconTextShowcasePage() {
  return (
          ...
          <div className="flex min-w-0 flex-1 flex-col gap-4">
            ...
            <p className="text-[14px] leading-[20px] text-black/40">
              <code className="rounded-[4px] bg-black/[0.04] px-[6px] py-[2px] font-mono text-[13px] text-black/70">
                @/components/base/frame-text-icon-text
              </code>
            </p>
          </div>
          <SnowUILogo />
